    ...
    <footer>
        <section>
            ...
            <div>
                ...
                <img src="https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto/portal/m/app_store.png" alt='Ios' />
            </div>
        </section>
        <section>
            ...
            {footerLinks.map((item,index)=>(
                <div className={`grid_${index+1} grid_item`} key={`footer-links-${index}`}>
                    <h3>{item.header}</h3>
                    {item.links.map((link,idx)=>(
                        <a href='http://www.w3.org/2000/svg' key={`link-${idx}`}>{(index===3&&idx===6)?<button><div>{link}</div><div><Icon icon="iconamoon:arrow-down-2-light" fontSize={20}/></div></button>:<>{link}</>}</a>
                    ))}
                </div>
            ))}
        </section>
